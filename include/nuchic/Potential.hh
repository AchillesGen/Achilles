...
class Potential {
    ...
    protected:
        PotentialVals stencil5(std::function<nuchic::PotentialVals(double)> f, double x, double h) const;
        PotentialVals stencil5second(std::function<nuchic::PotentialVals(double)> f, double x, double h) const;
        std::array<PotentialVals, 3> stencil5all(std::function<nuchic::PotentialVals(double)> f,
...
class SchroedingerPotential: public CooperPotential, RegistrablePotential<SchroedingerPotential> {
    ...
    private:
        static constexpr double wp = 1.0072545*nuchic::Constant::AMU;
        static constexpr double hc2 = nuchic::Constant::HBARC*nuchic::Constant::HBARC;
        size_t m_mode;
