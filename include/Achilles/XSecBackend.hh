...
class XSecBackend {
  public:
    ...
    virtual void SetupChannels(const ProcessInfo &, std::shared_ptr<Beam>, Integrand<Event> &,
                               PID) = 0;
...
class DefaultBackend : public XSecBackend, RegistrableBackend<DefaultBackend> {
  public:
    ...
    void SetupChannels(const ProcessInfo &, std::shared_ptr<Beam>, Integrand<Event> &,
                       PID) override;
...
class SherpaLeptonicBackend : public XSecBackend, RegistrableBackend<SherpaLeptonicBackend> {
  ...
  private:
    Currents CalcLeptonCurrents(const Event&, const ProcessInfo &) const;
