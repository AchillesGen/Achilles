...
enum class AdaptiveMapSplit { half, third, quarter };

class AdaptiveMap {
  public:
    AdaptiveMap() = default;
    AdaptiveMap(size_t dims, size_t bins) : m_dims(std::move(dims)), m_bins(std::move(bins)) {
    ...
    double width(size_t dim, size_t bin) const {
        return upper_edge(dim, bin) - lower_edge(dim, bin);
    }
...
template <> struct convert<achilles::AdaptiveMap> {
    static Node encode(const achilles::AdaptiveMap &rhs) {
        Node node;
        node["ndims"] = rhs.Dims();
        node["nbins"] = rhs.Bins();
    ...
    static bool decode(const Node &node, achilles::AdaptiveMap &rhs) {
        ...
        rhs = achilles::AdaptiveMap(ndims, nbins);
