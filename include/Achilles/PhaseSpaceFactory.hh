...
template <class Base, typename T> class PSFactory {
...
template <class Base, class Derived, typename T> class RegistrablePS {
  protected:
    RegistrablePS() = default;
    virtual ~RegistrablePS() {
        if(!m_registered) spdlog::error("Error registering phase space");
    }
...
const bool RegistrablePS<Base, Derived, T>::m_registered =
    RegistrablePS<Base, Derived, T>::Register();
