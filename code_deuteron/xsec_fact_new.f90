...
!     sina2    sin(alpha)**2, alpha being the angle between the 
      ...
      xk_z=xk*cosa
      p_4(1)=ek                 !initial nucleon
      p_4(2)=xk_x
      p_4(3)=xk_y
      p_4(4)=xk_z
      pp_4(1)=epf               !final nucleon
      pp_4(2)=xk_x
      pp_4(3)=xk_y
      pp_4(4)=xk_z+xq
    ...
    subroutine sigccc(sig,ig)   !cross section calculation
      use xsec_fact
      use dirac_matrices
      implicit none
      integer*4 :: ig
...
!.....compute sigma_mott [ fm^2 --> mb ]
      ...
      call det_Ja(ff1p,ff2p)    ! proton FF
      call det_res1b(rlp,rtp)   !longitudinal and tranverse contributions
      call det_Ja(ff1n,ff2n)    !same for neutron
