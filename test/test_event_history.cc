...
TEST_CASE("EventHistory Copy", "[EventHistory]") {
    ...
    achilles::Particle target(achilles::PID::carbon(), {achilles::ParticleInfo(achilles::PID::carbon()).Mass(), 0, 0, 0});
    achilles::Particle nuc_in(achilles::PID::neutron(), {918.00661011686168, 23.585833577703873, 85.332385429710143, 52.378992899809319});
    achilles::Particle beam(achilles::PID::nu_muon(), {1e4, 0, 0, 1e4});
    achilles::Particle neutrino(achilles::PID::nu_muon(), {5.3983343748755351e3, 0, 0, 5.3983343748755351e3});
    achilles::Particle nuc_out(achilles::PID::proton(), {9.6318094613481071e2, -1.8702102417549486e2, -4.7096228265225918e1, 1.0333739302250189e2});
    achilles::Particle lepton(achilles::PID::muon(), {5.3531600388575862e3, 2.1060685775319874e2, 1.3242861369493605e2, 5.3473759747528429e3});

    achilles::EventHistory history;
    history.AddVertex({8.1502403531109633e-13, 3.6163822359943019e-13, 1.0579315614474801e-12}, {target}, {nuc_in}, achilles::EventHistoryNode::StatusCode::target);
    history.AddVertex({8.1502403531109633e-13, 3.6163822359943019e-13, 1.0579315614474801e-12}, {beam}, {neutrino}, achilles::EventHistoryNode::StatusCode::beam);
    history.AddVertex({8.1502403531109633e-13, 3.6163822359943019e-13, 1.0579315614474801e-12}, {nuc_in, neutrino}, {nuc_out, lepton}, achilles::EventHistoryNode::StatusCode::primary);
    ...
    SECTION("Copy Constructor") {
        ...
        for (size_t i = 0; i < history.size(); ++i) {
            ...
            CHECK(original_node != copied_node); // Make sure it's a deep copy, not just the same pointer
            CHECK(original_node->Status() == copied_node->Status()); // Check node status is the same
            CHECK(original_node->ParticlesIn() == copied_node->ParticlesIn()); // Check particles in
            CHECK(original_node->ParticlesOut() == copied_node->ParticlesOut()); // Check particles out
        }
    }
    ...
    SECTION("Copying Does Not Affect Original") {
        ...
        achilles::Particle new_particle(achilles::PID::proton(), {achilles::ParticleInfo(achilles::PID::proton()).Mass(), 1, 1, 1});
        copied_history.AddVertex({1.0, 1.0, 1.0}, {new_particle}, {}, achilles::EventHistoryNode::StatusCode::primary);

        // Ensure that the copied history has the new vertex and the original does not
        CHECK(copied_history.size() == history.size() + 1); // The copied history should have one more vertex
