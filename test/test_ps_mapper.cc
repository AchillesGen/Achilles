...
class DummyHadron : public nuchic::HadronicBeamMapper,
                           nuchic::RegistrablePS<nuchic::HadronicBeamMapper, DummyHadron, size_t> {
    public:
        DummyHadron(size_t idx) : HadronicBeamMapper(idx, Name()) {}
        static std::string Name() { return "Dummy"; }
        static std::unique_ptr<nuchic::HadronicBeamMapper> Construct(const size_t &idx) {
            return std::make_unique<DummyHadron>(idx);
        }

        void GeneratePoint(std::vector<nuchic::FourVector> &p, const std::vector<double>&) override {
            p[HadronIdx()] = {nuchic::Constant::mN, 0, 0, 0};
        }
        double GenerateWeight(const std::vector<nuchic::FourVector>&, std::vector<double>&) override {
            return 1.0;
        }
...
class DummyFS : public nuchic::FinalStateMapper,
                       nuchic::RegistrablePS<nuchic::FinalStateMapper, DummyFS, std::vector<double>> {
    public:
        DummyFS(const std::vector<double>&) : FinalStateMapper(2) {}
        static std::string Name() { return "Dummy"; }
        static std::unique_ptr<nuchic::FinalStateMapper> Construct(const std::vector<double> &m) {
            return std::make_unique<DummyFS>(m);
        }

        void GeneratePoint(std::vector<nuchic::FourVector> &p, const std::vector<double>&) override {
            p[2] = {nuchic::Constant::mN, 0, 0, 0};
            p[3] = {nuchic::Constant::mN, 0, 0, 0};
        }
        double GenerateWeight(const std::vector<nuchic::FourVector>&, std::vector<double>&) override {
            return 1.0;
        }
...
TEST_CASE("PhaseSpaceBuilder", "[PhaseSpace]") {
    ...
    nuchic::FourVector beam_mom = {nuchic::Constant::mN, 0, 0, 0};
    std::vector<nuchic::FourVector> expected = {beam_mom, beam_mom, beam_mom, beam_mom};
    std::vector<nuchic::FourVector> output(4);
    auto mapper = nuchic::PSBuilder(2, 2).Beam(beam, 1)
                                         .Hadron("Dummy", {nuchic::Constant::mN2, 0.0})
                                         .FinalState("Dummy", {nuchic::Constant::mN2, 0.0})
                                         .build();
    std::vector<double> rans(2);
    std::vector<double> beam_rans;
    std::set<nuchic::PID> beam_id{nuchic::PID::electron()};
    REQUIRE_CALL(*beam, BeamIDs())
        .TIMES(1)
        .LR_RETURN((beam_id));
    REQUIRE_CALL(*beam, Flux(nuchic::PID::electron(), beam_rans))
        .TIMES(1)
        .LR_RETURN((beam_mom));
...
TEST_CASE("PhaseSpaceMapper", "[PhaseSpace]") {
    ...
    std::vector<double> masses{0, nuchic::Constant::mN2};
    size_t beam_dims = 0, hadron_dims = 4, final_state_dims = 2;
    static constexpr double sqrts = 200;
    nuchic::FourVector beam_mom = {100, 0, 0, -100};
    nuchic::FourVector hadron_mom = {100, 0, 0, 100};
    std::vector<nuchic::FourVector> final_state_mom ={{sqrts/2, sqrts/2*sin(M_PI/4), 0, sqrts/2*cos(M_PI/4)},
                                                      {sqrts/2, -sqrts/2*sin(M_PI/4), 0, -sqrts/2*cos(M_PI/4)}}; 
    std::vector<nuchic::FourVector> mom_out = {hadron_mom, beam_mom, final_state_mom[0], final_state_mom[1]};
    std::vector<nuchic::FourVector> mom(4), mom2(4), mom3(4);
    mom2[1] = beam_mom;
    mom3[0] = hadron_mom;
    mom3[1] = beam_mom;
