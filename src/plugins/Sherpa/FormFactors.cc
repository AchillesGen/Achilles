...
FormFactorInterface::FormFactorInterface
(const Vertex_Key &key,const std::string &id):
...
#endif
  double result(0.);
  FormFactor::Values ffs((*p_ff)(Q2*sqr(1._GeV)/*in MeV*/));
  if (m_id=="F1p") result=ffs.F1p;
  else if (m_id=="F2p") result=ffs.F2p;
  else if (m_id=="F1n") result=ffs.F1n;
  else if (m_id=="F2n") result=ffs.F2n;
  else if (m_id=="FA") result=ffs.FA;
  else if (m_id=="FCoh") result=ffs.Fcoh;
  else THROW(not_implemented,"Form factor not found");
#ifdef DEBUG__BG
...
#define FORM_FACTOR_IMPLEMENTATION(NAME)				\
  namespace achilles {							\
    class NAME: public Getter_Function					\
    <Form_Factor,Vertex_Key,std::less<std::string> > {			\
    ...
    public:								\
    NAME(const std::string &name): ATOOLS::Getter_Function		\
      <Form_Factor,Vertex_Key,std::less<std::string> >(name) {}         \
    };									\
