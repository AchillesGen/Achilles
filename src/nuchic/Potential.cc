...
PotentialVals CooperPotential::operator()(const double &plab, const double &radius) {
    ...
    const auto x = e, x2 = x*x, x3 = x*x2, x4 = x2*x2;
...
double CooperPotential::CalcTerm(double prefact, double real, double acb, double imag, double radius) const {
...
double CooperPotential::CalcTermSurf(double prefact, double real, double acb, double imag, double radius) const {
    const auto s1 = sech(real*acb/imag);
    const auto s2 = sech(radius/imag);
    const auto prod = s1*s2;
    const auto a = s1 - prod;
