...
!
!       mode_in =1  neutrino CC scattering on nucleon
!               =3  anti-neutrino CC scattering on nucleon
!               =-1  neutrino NC scattering on nucleon
!               =-3  anti-neutrino NC scattering on nucleon
...
!
     ...
     &  )
      ...
      common/chdat1/njLs,jpind(maxlsj) ,Lpind(maxlsj)
     &                  ,ispind(maxlsj),itpind(maxlsj)
      parameter (maxwcm=70,maxq2=30,ilmax=1,mbs=1)
...
!
      if(isl==1)then
      open(10,file='data/sl_EW.dat',form='formatted',status='old')
      else
...
!
! isoscalar vector current for NC      
!
      if(mode.le.-1)then
      do 720 ipw=1,njLs
      tpin=dble(itpind(ipw))/2
