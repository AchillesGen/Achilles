...
std::array<DoubleExponential::DEPoint, DoubleExponential::_size> DoubleExponential::table{};
std::array<size_t, DoubleExponential::_phases+1> DoubleExponential::ip{};
bool DoubleExponential::init = false;

double DoubleExponential::Integrate(const double &x1, const double &x2,
                                    double &ceps1, double &ceps2) {
    ...
    for(size_t k = 1; k <= _phases; ++k) {
        ...
        if(k <= 2)
            ...
            err = pow(10.0, static_cast<int>(std::min(0.0, max)));
        }

        spdlog::trace("Iteration {}: I = {} +/- {}, nevals = {}", k, s1, err, evals);

        if(k > 3 && err < eps1) {
            spdlog::debug("DoubleExponential: I = {} +/- {}, nevals = {}", s1, err, evals);
            return s1;
        }
        if(k >= 3 && err < eps2) {
            spdlog::debug("DoubleExponential: I = {} +/- {}, nevals = {}", s1, err, evals);
    ...
    spdlog::warn("DoubleExponential: Failed to converge, I = {} +/- {}, nevals = {}", s1, err, evals);
    return s1;
}
...
std::vector<double> DoubleExponential::IntegrateVec(const double &x1, const double &x2,
    ...
    for(size_t k = 1; k <= _phases; ++k) {
        ...
        for(size_t i = 0; i < _size; i += k1) {
            if(i % k2 != 0 || k == 1) {
                ...
                if(log1) {
                    t1 = FunctionVec(xx1);
                    evals++;
                    std::transform(t1.begin(), t1.end(), tw1.begin(), [&](const double x){
                            return x*table[i].weights;
                    });
                    if(std::abs(*std::max_element(tw1.begin(), tw1.end())) < ceps1) iz1++;
                    else iz1 = 0;
                } else {
                ...
                if(i > 0 && log2) {
                    t2 = FunctionVec(xx2);
                    evals++;
                    std::transform(t2.begin(), t2.end(), tw2.begin(), [&](const double x){
                            return x*table[i].weights;
                    });
                    if(std::abs(*std::max_element(tw2.begin(), tw2.end())) < ceps1) iz2++;
                    else iz2 = 0;
                } else {
        ...
        auto diff1 = s1;
        auto diff2 = s1;
        std::transform(diff1.begin(),diff1.end(),s2.begin(),diff1.begin(),std::minus<double>());
        std::transform(diff1.begin(),diff1.end(),s3.begin(),diff1.begin(),std::minus<double>());
        ...
        spdlog::trace("Iteration {}: I = {} +/- {}, nevals = {}", k, s1[0], err, evals);

        if(k > 3 && err < eps1) {
            spdlog::debug("DoubleExponential: I = {} +/- {}, nevals = {}", s1[0], err, evals);
            return s1;
        }
        if(k >= 3 && err < eps2) {
            spdlog::debug("DoubleExponential: I = {} +/- {}, nevals = {}", s1[0], err, evals);
    ...
    spdlog::warn("DoubleExponential: Failed to converge, I = {} +/- {}, nevals = {}", s1[0], err, evals);
    return s1;
}
