...
nuchic::Particles nuchic::MetropolisData::ElasticGen(const std::array<double, 2> &rans,
...
Particles nuchic::MetropolisData::DoublePion(ChargeMode, const std::vector<double> &rans,
                                          const Particle &part1,
                                          const Particle &part2) {
...
Particles nuchic::NucleonData::Elastic(ChargeMode charge,
    ...
    return ElasticGen(rans, A, B, part1, part2);
}

std::vector<nuchic::PID> nuchic::NucleonData::PionCharge(ChargeMode, const double &ran,
                                                         const PID &pid1,
...
Particles nuchic::PionData::CEX(ChargeMode charge,
    ...
    Particles outgoing = ElasticGen(rans, A, B, part1, part2);
...
nuchic::Metropolis::Metropolis(const YAML::Node &node) {
    // Set the pion production mode
    ProductionMethod production; 
    std::string mode = node["Production"].as<std::string>();
...
Particles nuchic::Metropolis::GenerateFinalState(const Particle &part1,
                                                 const Particle &part2) const {
    auto mode = GetMode(part1, part2);
    ...
    if(IsInelastic(mode, kinetic, Random::Instance().Uniform(0.0, 1.0))) {
        if(IsCEX(mode, kinetic, Random::Instance().Uniform(0.0, 1.0))) {
            outgoing = CEX(mode, kinetic, part1CM, part2CM);
        } else if(energy > ParticleInfo(PID::pionp()).Mass()) {
            if(IsSinglePion(mode, kinetic, Random::Instance().Uniform(0.0, 1.0))) {
                ...
                outgoing = Elastic(mode, kinetic, part1CM, part2CM);
            }
        } else 
            outgoing = Elastic(mode, kinetic, part1CM, part2CM);
    } else 
...
std::vector<double> nuchic::Metropolis::XSec(nuchic::InteractionType mode, const Particle &part1, const Particle &part2) const {
    return GetProbe(mode.first) -> XSec(mode.second, part1, part2);
}

// TODO: Figure out how to handle the absorption
Particles nuchic::Metropolis::Absorption(nuchic::InteractionType,
                                         const Particle &,
...
Particles nuchic::Metropolis::Elastic(nuchic::InteractionType mode,
                                      const double &kinetic,
                                      const Particle &part1,
                                      const Particle &part2) const {
...
Particles nuchic::Metropolis::CEX(nuchic::InteractionType mode,
                                  const double &kinetic,
                                  const Particle &part1, 
                                  const Particle &part2) const {
    std::array<double, 2> rans{};
...
Particles nuchic::Metropolis::SinglePion(nuchic::InteractionType mode,
                                         const Particle &part1, 
                                         const Particle &part2) const {
    // Rambo: 4*n = 12
    // Channel select: 1
...
Particles nuchic::Metropolis::DoublePion(nuchic::InteractionType mode,
                                         const Particle &part1, 
                                         const Particle &part2) const {
    // Rambo: 4*n = 16
    // Channel select: 1
...
bool nuchic::Metropolis::IsAbsorption(nuchic::InteractionType mode, const std::vector<double> &xsec, const double &ran) const {
    return GetProbe(mode.first) -> IsAbsorption(xsec, ran);
}

bool nuchic::Metropolis::IsInelastic(nuchic::InteractionType mode, const double &kinetic, const double &ran) const {
    return GetProbe(mode.first) -> IsInelastic(mode.second, kinetic, ran);
}

bool nuchic::Metropolis::IsCEX(nuchic::InteractionType mode, const double &kinetic, const double &ran) const {
    return GetProbe(mode.first) -> IsCEX(mode.second, kinetic, ran);
}

bool nuchic::Metropolis::IsSinglePion(nuchic::InteractionType mode, const double &kinetic, const double &ran) const {
