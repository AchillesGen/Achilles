...
void nuchic::Vegas::operator()(const Func<double> &func) {
...
void nuchic::Vegas::Optimize(const Func<double> &func) {
...
double nuchic::Vegas::GenerateWeight(const std::vector<double> &rans) const {
    return grid.GenerateWeight(rans); 
}

void nuchic::Vegas::Adapt(const std::vector<double> &train_data) {
    grid.Adapt(params.alpha, train_data);
}

void nuchic::Vegas::Refine() {
    grid.Split();
    params.ncalls *= 2;
}

nuchic::VegasSummary nuchic::Vegas::Summary() const {
...
void nuchic::Vegas::PrintIteration() const {
