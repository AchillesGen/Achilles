...
class PyInteraction : public Interactions {
  public:
    ...
    ThreeVector MakeMomentum(bool samePID, const double &pcm,
        ...
        PYBIND11_OVERLOAD_PURE(ThreeVector,  // Return type
                               Interactions, // Parent class
...
void InteractionsModule(py::module &m) {
    py::class_<Interactions, PyInteraction, std::shared_ptr<Interactions>>(m, "Interactions")
        .def(py::init<>())
        .def_static("create",
                    [](const YAML::Node &data) { return InteractionFactory::Create(data); })
