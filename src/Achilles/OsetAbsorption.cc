...
double OsetAbsCrossSection::AbsorptionCrossSection(Event &event, size_t part1, size_t) const {
    ...
    auto sqrts = sqrt(pion.Mass()*pion.Mass() + 2.0 * Constant::mN * pion.E() + pow(Constant::mN,2));
    ...
    auto coupling_factor = fCouplingConstant / pion.Mass() / pion.Mass(); // (f*/m_pi)^2, e.g. eq. 2.6

    // Reduce delta width due to pauli blocking see eq. 2.7 and sec. 2.3
    auto reduced_halfwidth = constFactor * coupling_factor * Constant::mN * momentum_cms *
                         momentum_cms2 / sqrts * DeltaReduction(pion.E(), pion.Mass(), momentum_cms, fermi_energy, sqrts);
    ...
    auto self_energy_total = absNN + absNNN + QuadraticFunction (x, fCoefCQ) * pow (densityFraction, alpha);
    ...
    const double pXsecCommon = fNormFactor * coupling_factor * delta_propagator2 * momentum_cms2 / pion_momentum;
    ...
    const double sXsecAbsorption = sAbsorptionFactor / pion_momentum * density * (1.0 + pion.E() / 2.0 / Constant::mN) / pow (pion.Mass() / 197.327, 4.0);
...
double OsetAbsCrossSection::DeltaReduction (const double pion_E, const double pion_mass, const double momentum_cms, const double fermi_energy, const double sqrts) const
{
  ...
  const double mu0 = (deltaEnergy * energy_cms - fermi_energy * sqrts) /
                     pion_p / momentum_cms;
