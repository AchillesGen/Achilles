...
Histogram::Histogram(const size_t &nbins, const double &lower, const double &upper,
    ...
    for(size_t i = 0; i <= nbins; ++i) { binedges[i] = lower + static_cast<double>(i) * binsize; }
}

Histogram::Histogram(std::vector<double> binedges_, std::string name_, std::string path_)
    ...
    for(size_t i = 0; i < binvals.size(); ++i) {
        if(binedges[i] > binedges[i + 1])
            throw std::runtime_error("Bin edges must be in increasing order!");
        if(binedges[i] == binedges[i + 1])
            throw std::runtime_error("Bin edges must not be identical!");
    }
}
...
double Histogram::Integral(size_t lower, size_t upper) const {
    ...
    if(upper > binvals.size()) throw std::runtime_error("Invalid range for histogram integration");
    ...
    *out << fmt::format("{:^15} {:^15} {:^15} {:^15}\n", "lower edge", "upper edge", "value",
    ...
    for(size_t i = 0; i < binvals.size(); ++i) {
        *out << fmt::format("{:< 15.6e} {:< 15.6e} {:< 15.6e} {:< 15.6e}\n", binedges[i],
                            binedges[i + 1], binvals[i], std::sqrt(errors[i]));
    }
}
...
YODAHistogram::YODAHistogram(const size_t &nbins, const double &lower, const double &upper,
                             const std::string &name_, const std::string &path_) {
    name = name_;
    path = path_;
    histogram = YODA::Histo1D(nbins, lower, upper, path, name);
}

YODAHistogram::YODAHistogram(const std::vector<double> &binedges, const std::string &name_,
...
ROOTHistogram::ROOTHistogram(const size_t &nbins, const double &lower, const double &upper,
                             const std::string &_name, const std::string &) {
    histogram = new TH1D(_name.c_str(), "", static_cast<int>(nbins), lower, upper);
}

ROOTHistogram::ROOTHistogram(const std::vector<double> &_binedges, const std::string &_name,
                             const std::string &) {
...
bool HistogramCollection::AddHistogram(const size_t& nbins, const double& lower,
    ...
    switch(outputMode) {
        ...
        case HistOut::YODA:
            ...
            break;
#else
            throw std::runtime_error("ResBos2 not compiled with YODA histogram
support"); #endif
        ...
        case HistOut::ROOT:
            ...
            break;
#else
            throw std::runtime_error("ResBos2 not compiled with ROOT histogram
support"); #endif
