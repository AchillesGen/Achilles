...
double MBAmplitudes::CSTotal(double W, int ichan, int twoIm, int twoIb)
{
	...
	for (int iI = 0 ; iI < 2 ; iI++)
	{
		...
		for( int iJ = 0; iJ < 2 ; iJ++)
		{
			...
			CStot += CGs[iI]*Jfac*(A_LJI[ichan][ichan][iW_m+1][iL][iJ][iI]).imag()*cofmax;
		}	
...
void MBAmplitudes::SetOpenCChannels()
{
	for (int iCChan = 0 ; iCChan < nCchan ; iCChan++)
	{
		...
		for (int i_f = 0 ; i_f < nCchan ; i_f++)
		{
			...
			if ( (std::abs(CG1_f*CG1_i) < 1.0*1e-4)  && (std::abs(CG3_f*CG3_i) < 1.0*1e-4) ){continue;} //should not happen in principle
...
//	if (W > w_vec[iMB_i][iMB_f][nW-1]){ return 0.; }
	...
	for (int Ln = 0 ; Ln < Lmax+1 ; Ln++)
	{
		...
		{
			double Lmd = 1.*Lm;
			double ampG2 = 2.*std::real( std::conj(A_LJ[Ln][0] - A_LJ[Ln][1]) * (A_LJ[Lm][0] - A_LJ[Lm][1]));
			std::complex<double> AH = std::conj( Lnd*A_LJ[Ln][0] + (Lnd+1.)*A_LJ[Ln][1]) * (Lmd* A_LJ[Lm][0] + (Lmd+1.) * A_LJ[Lm][1]);
