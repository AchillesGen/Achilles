...
using achilles::D2SpectralMapper;
...
D2SpectralMapper::D2SpectralMapper(const ProcessInfo &info, size_t idx)
    : HadronicBeamMapper(info, idx) {
    SetMasses(info.Masses());
}

void D2SpectralMapper::GeneratePoint(std::vector<FourVector> &point,
                                     const std::vector<double> &rans) {
    // Generate inital nucleon state
    double pmin = point[0].E() - sqrt(pow(point[0].E(), 2) + 2 * point[0].E() * Constant::mN +
                                      Constant::mN2 - Smin());
    ...
    // TODO: Define binding energy
    double binding = 0;
    const double energy = sqrt(mom * mom + Constant::mN2) - binding;
    point[HadronIdx()] = {energy, mom * sinT * cos(phi), mom * sinT * sin(phi), mom * cosT};
#ifdef ACHILLES_EVENT_DETAILS
    Mapper<FourVector>::Print(__PRETTY_FUNCTION__, point, rans);
    spdlog::trace("  point[0] = {}", point[0]);
    spdlog::trace("  dp = {}", dp);
...
double D2SpectralMapper::GenerateWeight(const std::vector<FourVector> &point,
                                        std::vector<double> &rans) {
    double pmin = point[0].E() - sqrt(pow(point[0].E(), 2) + 2 * point[0].E() * Constant::mN +
                                      Constant::mN2 - Smin());
    double pmax = point[0].E() + sqrt(pow(point[0].E(), 2) + 2 * point[0].E() * Constant::mN +
    ...
    double wgt = 1.0 / point[1].P2() / dp / dCos / dPhi;
#ifdef ACHILLES_EVENT_DETAILS
    Mapper<FourVector>::Print(__PRETTY_FUNCTION__, point, rans);
    spdlog::trace("  Weight: {}", wgt);
    spdlog::trace("  dp: {}", dp);
