...
using achilles::MecSpectralMapper;
...
double IntfSpectralMapper::GenerateWeight(const std::vector<FourVector> &point,
    ...
    double wgt = 1.0 / point[1].P2() / dp / dCos / dPhi / dE / point.back().P2() / dp2 / dCos2 / dPhi;
    Mapper<FourVector>::Print(__PRETTY_FUNCTION__, point, rans);
    spdlog::trace("  Weight: {}", wgt);
    spdlog::trace("  dp: {}", dp);
    spdlog::trace("  dCos: {}", dCos);
...
void MecSpectralMapper::GeneratePoint(std::vector<FourVector> &point,
                                     const std::vector<double> &rans) {
    // Generate 2nd nucleon momentum from a flat distribution
    double p2 = dp2*rans[4];
    double cosT2 = dCos2*rans[5] - 1.;
    double sinT2 = sqrt(1 - cosT2*cosT2);
    double phi2 = dPhi*rans[6];
    double E2 = dE2*rans[7];

    spdlog::debug("p2 = {}, cosT = {}, phi = {}, E2 = {}", p2, cosT2, phi2, E2);

    ThreeVector pmom2 = {p2 * sinT2 * cos(phi2), p2 * sinT2 * sin(phi2), p2 * cosT2};
    point[HadronIdx()+1] = {Constant::mN - E2, pmom2[0], pmom2[1], pmom2[2]};

    spdlog::debug("p2 fourvector = {},{},{},{}", point[HadronIdx()+1].E(),point[HadronIdx()+1].X(),point[HadronIdx()+1].Y(),point[HadronIdx()+1].Z());
...
double MecSpectralMapper::GenerateWeight(const std::vector<FourVector> &point,
                                        std::vector<double> &rans) {
    double pmin = point[0].E() - sqrt(pow(point[0].E(), 2) + 2 * point[0].E() * Constant::mN +
                                      Constant::mN2 - Smin());
    double pmax = point[0].E() + sqrt(pow(point[0].E(), 2) + 2 * point[0].E() * Constant::mN +
    ...
    double spectator_momentum = point[HadronIdx()+1].P();
    rans[4] = spectator_momentum/dp2;
    rans[5] = (point[HadronIdx()+1].CosTheta() + 1) / dCos2;
    rans[6] = point[HadronIdx()+1].Phi() / dPhi;
    rans[7] = point[HadronIdx()+1].E()/dE2;
    ...
    double wgt = 1.0 / point[1].P2() / dp / dCos / dPhi / dE / point[HadronIdx()+1].P2() / dp2 / dCos2 / dPhi / dE2;
    Mapper<FourVector>::Print(__PRETTY_FUNCTION__, point, rans);
    spdlog::trace("  Weight: {}", wgt);
    spdlog::trace("  dp: {}", dp);
    spdlog::trace("  dCos: {}", dCos);
