...
using achilles::Potential;
using achilles::PotentialVals;
using achilles::SchroedingerPotential;
...
PotentialVals Potential::stencil5(std::function<achilles::PotentialVals(double)> f, double x,
                                  double h) const {
    auto fp2h = f(x + 2 * h);
    auto fph = f(x + h);
    auto fm2h = f(x - 2 * h);
    ...
    achilles::PotentialVals results{};
    double den = 12 * h;
...
PotentialVals Potential::stencil5second(std::function<achilles::PotentialVals(double)> f, double x,
                                        double h) const {
    auto fp2h = f(x + 2 * h);
    auto fph = f(x + h);
    auto fm2h = f(x - 2 * h);
    ...
    results.rvector =
        (-fp2h.rvector + 16 * fph.rvector - 30 * f0.rvector + 16 * fmh.rvector - fm2h.rvector) /
        den;
    results.ivector =
        (-fp2h.ivector + 16 * fph.ivector - 30 * f0.ivector + 16 * fmh.ivector - fm2h.ivector) /
        den;
    results.rscalar =
        (-fp2h.rscalar + 16 * fph.rscalar - 30 * f0.rscalar + 16 * fmh.rscalar - fm2h.rscalar) /
        den;
    results.iscalar =
        (-fp2h.iscalar + 16 * fph.iscalar - 30 * f0.iscalar + 16 * fmh.iscalar - fm2h.iscalar) /
        den;
    return results;
}

std::array<PotentialVals, 3>
Potential::stencil5all(std::function<achilles::PotentialVals(double)> f, double x, double h) const {
    auto fp2h = f(x + 2 * h);
    auto fph = f(x + h);
    auto fm2h = f(x - 2 * h);
    auto fmh = f(x - h);
    ...
    std::array<achilles::PotentialVals, 3> results{};
    results[0] = f0;
...
achilles::SquareWellPotential::Construct(std::shared_ptr<Nucleus>& nuc, const
...
PotentialVals CooperPotential::evaluate(const double &plab, const double &radius) const {
    ...
    const auto sumr =
        -100. * (Data(1, 1) + Data(1, 2) * x + Data(1, 3) * x2 + Data(1, 4) * x3 + Data(1, 5) * x4 +
                 Data(1, 6) * y + Data(1, 7) * y2 + Data(1, 8) * y3 + Data(20, 1) * y4);
    const auto rv1 =
        cv1b * (Data(2, 1) + Data(2, 2) * x + Data(2, 3) * x2 + Data(2, 4) * x3 + Data(2, 5) * x4 +
                Data(2, 6) * y + Data(2, 7) * y2 + Data(2, 8) * y3 + Data(21, 1) * y4);
    const auto av1 = av1b * (Data(3, 1) + Data(3, 2) * x + Data(3, 3) * x2 + Data(3, 4) * x3 +
                             Data(3, 5) * x4 + Data(3, 6) * y + Data(3, 7) * y2 + Data(3, 8) * y3);
    const auto sumi =
        -15.0 * (Data(4, 1) + Data(4, 2) * x + Data(4, 3) * x2 + Data(4, 4) * x3 + Data(4, 5) * x4 +
                 Data(4, 6) * y + Data(4, 7) * y2 + Data(4, 8) * y3 + Data(20, 2) * y4);
    const auto rv2 =
        cv2b * (Data(5, 1) + Data(5, 2) * x + Data(5, 3) * x2 + Data(5, 4) * x3 + Data(5, 5) * x4 +
                Data(5, 6) * y + Data(5, 7) * y2 + Data(5, 8) * y3 + Data(21, 2) * y4);
    const auto av2 = av2b * (Data(6, 1) + Data(6, 2) * x + Data(6, 3) * x2 + Data(6, 4) * x3 +
                             Data(6, 5) * x4 + Data(6, 6) * y + Data(6, 7) * y2 + Data(6, 8) * y3);
    const auto diffr =
        700. * (Data(7, 1) + Data(7, 2) * x + Data(7, 3) * x2 + Data(7, 4) * x3 + Data(7, 5) * x4 +
                Data(7, 6) * y + Data(7, 7) * y2 + Data(7, 8) * y3 + Data(20, 3) * y4);
    const auto rs1 =
        cs1b * (Data(8, 1) + Data(8, 2) * x + Data(8, 3) * x2 + Data(8, 4) * x3 + Data(8, 5) * x4 +
                Data(8, 6) * y + Data(8, 7) * y2 + Data(8, 8) * y3 + Data(21, 3) * y4);
    const auto as1 = as1b * (Data(9, 1) + Data(9, 2) * x + Data(9, 3) * x2 + Data(9, 4) * x3 +
                             Data(9, 5) * x4 + Data(9, 6) * y + Data(9, 7) * y2 + Data(9, 8) * y3);
    const auto diffi = -150 * (Data(10, 1) + Data(10, 2) * x + Data(10, 3) * x2 + Data(10, 4) * x3 +
                               Data(10, 5) * x4 + Data(10, 6) * y + Data(10, 7) * y2 +
                               Data(10, 8) * y3 + Data(20, 4) * y4);
    const auto rs2 = cs2b * (Data(11, 1) + Data(11, 2) * x + Data(11, 3) * x2 + Data(11, 4) * x3 +
                             Data(11, 5) * x4 + Data(11, 6) * y + Data(11, 7) * y2 +
                             Data(11, 8) * y3 + Data(21, 4) * y4);
    const auto as2 =
        as2b * (Data(12, 1) + Data(12, 2) * x + Data(12, 3) * x2 + Data(12, 4) * x3 +
                Data(12, 5) * x4 + Data(12, 6) * y + Data(12, 7) * y2 + Data(12, 8) * y3);
    ...
    const auto wv2 = -100.0 * (Data(13, 1) + Data(13, 2) * x + Data(13, 3) * x2 + Data(13, 4) * x3 +
                               Data(13, 5) * x4 + Data(13, 6) * y + Data(13, 7) * y2 +
                               Data(13, 8) * y3 + Data(20, 5) * y4);
    // const auto rv22 =       (Data(14, 1)+Data(14, 2)*x+Data(14,
    // 3)*x2+Data(14, 4)*x3+Data(14, 5)*x4); const auto av22 =   0.7*(Data(15,
    // 1)+Data(15, 2)*x+Data(15, 3)*x2+Data(15, 4)*x3+Data(15, 5)*x4);

    const auto ws2 = 100.0 * (Data(16, 1) + Data(16, 2) * x + Data(16, 3) * x2 + Data(16, 4) * x3 +
                              Data(16, 5) * x4 + Data(16, 6) * y + Data(16, 7) * y2 +
                              Data(16, 8) * y3 + Data(20, 6) * y4);
    // const auto rs22 =       (Data(17, 1)+Data(17, 2)*x+Data(17,
    // 3)*x2+Data(17, 4)*x3+Data(17, 5)*x4); const auto as22 =   0.7*(Data(18,
    // 1)+Data(18, 2)*x+Data(18, 3)*x2+Data(18, 4)*x3+Data(18, 5)*x4);
    ...
    const auto rva2 = CalcTerm(recv * wv, rv2, acb, av2, radius) +
                      CalcTermSurf(recv * wv2, rv22, acb, av22, radius);
    const auto rsa1 = CalcTerm(recs * vs, rs1, acb, as1, radius);
    const auto rsa2 = CalcTerm(recs * ws, rs2, acb, as2, radius) +
                      CalcTermSurf(recs * ws2, rs22, acb, as22, radius);
...
achilles::PotentialVals SchroedingerPotential::operator()(double plab, double radius) const {
    ...
    const auto ucr1 = 2 * epcm * u2 + 2 * wp * u1 - pow(u2, 2) + pow(w2, 2) + pow(u1, 2) -
                      pow(w1, 2) - 2 * ac * u2;
    ...
    const auto udr3 =
        0.75 *
        ((pow(adr, 2) - pow(adi, 2)) * (pow(aab, 2) - pow(aai, 2)) + 4.0 * adr * adi * aab * aai) /
    ...
    const auto udi3 =
        1.5 * (adr * adi * (pow(aab, 2) - pow(aai, 2)) - (pow(adr, 2) - pow(adi, 2)) * aab * aai) /
        (pow(pow(aab, 2) - pow(aai, 2), 2) + 4.0 * pow(aab, 2) * pow(aai, 2));
