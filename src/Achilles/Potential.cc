...
PotentialVals Potential::stencil5(std::function<achilles::PotentialVals(double)> f, double x, double h) const {
    auto fp2h = f(x + 2*h);
    auto fph = f(x + h);
    auto fm2h = f(x - 2*h);
    auto fmh = f(x - h);
...
PotentialVals Potential::stencil5second(std::function<achilles::PotentialVals(double)> f,
...
std::array<PotentialVals, 3> Potential::stencil5all(std::function<achilles::PotentialVals(double)> f,
                                                    double x, double h) const {
    auto fp2h = f(x + 2*h);
    auto fph = f(x + h);
    auto fm2h = f(x - 2*h);
...
/*
std::unique_ptr<Potential> achilles::SquareWellPotential::Construct(std::shared_ptr<Nucleus>& nuc,
                                                                  const YAML::Node&) {
    return std::make_unique<SquareWellPotential>(nuc); 
}

PotentialVals achilles::SquareWellPotential::operator()(const double&, const double &r) const {
