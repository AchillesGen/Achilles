...
std::string QESpectral::PhaseSpace(PID nuc_id) const {
	is_hydrogen=nuc_id==PID::hydrogen();
	is_free_neutron=nuc_id==PID::free_neutron();
	return (is_hydrogen||is_free_neutron)?Coherent::Name():PSName();
}
...
std::string DeepInelastic::PhaseSpace(PID nuc_id) const {
	is_hydrogen=nuc_id==PID::hydrogen();
	is_free_neutron=nuc_id==PID::free_neutron();
	return (is_hydrogen||is_free_neutron)?"DISSingleNucleon":PSName();
}
...
NuclearModel::Currents DeepInelastic::CalcCurrents(const std::vector<Particle>&,
												   const std::vector<Particle>&,
												   const std::vector<Particle>&, const FourVector&,
												   const FFInfoMap&) const {
	throw std::runtime_error("DeepInelastic::CalcCurrents(): Only handled by Sherpa Backend");
}