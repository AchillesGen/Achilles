...
PSBuilder &PSBuilder::Beam(std::shared_ptr<achilles::Beam> beam) {
    phase_space->lbeam = std::make_shared<BeamMapper>(beam);
    phase_space->lbeam->SetMasses(m_info.Masses());
    return *this;
}

PSBuilder &PSBuilder::Hadron(const std::string &mode) {
    phase_space->hbeam = Factory<HadronicBeamMapper, const ProcessInfo &>
		::Initialize(mode,m_info);
    return *this;
}
