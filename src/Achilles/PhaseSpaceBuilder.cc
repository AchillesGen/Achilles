...
PSBuilder &PSBuilder::FinalState(const std::string &channel,
    ...
    phase_space->finalstate =
        Factory<FinalStateMapper, std::vector<double>, size_t>::Initialize(channel, m_info.Masses(),phase_space->FinalStateIdx());
...
PSBuilder &PSBuilder::SherpaFinalState(const std::string &channel) {
    ...
    phase_space->finalstate = std::make_unique<SherpaMapper>(m_nlep + m_nhad - 2,phase_space->FinalStateIdx(), std::move(sherpaMap));
    return *this;
}

PSBuilder &PSBuilder::GenFinalState(std::unique_ptr<PHASIC::Channels> channel) {
    phase_space->finalstate = std::make_unique<SherpaMapper>(m_nlep + m_nhad - 2,phase_space->FinalStateIdx(), std::move(channel));
    return *this;
}
#endif
