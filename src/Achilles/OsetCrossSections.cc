...
double OsetCrossSection::AbsCrossSection(Event &event, size_t part1, size_t part2) const {
    ...
    */
    // ----- ABSORPTION ----- //
    auto pXsecCommon = PXSecCommon(pion.E(), pion.Mass(), cms_mom, fermi_momentum, sqrts, density);

    // constant factor for p-wave absorption cross section
    static const double pAbsorptionFactor = 4.0 / 9.0;

    auto self_energy_absorption = SelfEnergyAbsNN(pion_kinetic_energy, pion.Mass(), density) + SelfEnergyAbsNNN(pion_kinetic_energy, pion.Mass(), density);
...
std::map<std::pair<PID,PID>,double> OsetCrossSection::QECrossSection(Event &event, size_t part1, size_t part2) const {
    // Get pion and nucleus information
    const auto &pion = event.Hadrons()[part1];
    const auto &nucleon = event.Hadrons()[part2];
    auto density = event.CurrentNucleus()->Rho(nucleon.Position().Magnitude());
    ...
    double protfrac = (event.CurrentNucleus()->NNeutrons() - event.CurrentNucleus()->NProtons() ) / event.CurrentNucleus()->NNucleons();
    ...
    */
    ...
    const double pXsecTotalQel = pAbsorptionFactor * PXSecCommon(pion.E(), pion.Mass(), cms_mom, fermi_momentum, sqrts, density) * ReducedHalfWidth(pion.E(), pion.Mass(), cms_mom, fermi_momentum, sqrts);

    const double ksi = (sqrts - Constant::mN - pion.Mass()) / pion.Mass();

    const double sXsecTotalQel = QuadraticFunction(ksi, fCoefSigma) / pow(pion.Mass(),2) * fNormFactor;
    ...
    QelCrossSections[{PID::pionp(),PID::pionp()}] = sXsecTotalQel * (A - protfrac * B) + pXsecTotalQel * (5. - 4 * protfrac) / 6.;
    QelCrossSections[{PID::pionp(),PID::pion0()}] = sXsecTotalQel * (1. + protfrac) * C + pXsecTotalQel * (1 + protfrac) / 6.;
    QelCrossSections[{PID::pionp(),-PID::pionp()}] = sXsecTotalQel * 0. + 0.;
    QelCrossSections[{PID::pion0(),PID::pionp()}] = sXsecTotalQel * (1. - protfrac) * C + pXsecTotalQel * (1. - protfrac) / 6;
...
double OsetCrossSection::SelfEnergyAbsNN(const double pion_KE, const double pion_mass, const double density) const {
...
double OsetCrossSection::SelfEnergyAbsNNN(const double pion_KE, const double pion_mass, const double density) const {
    auto x = KEFraction(pion_KE,pion_mass);
    auto beta = QuadraticFunction(x, fCoefBeta);
...
double OsetCrossSection::SelfEnergyQE(const double pion_KE, const double pion_mass, const double density) const {
    auto x = KEFraction(pion_KE,pion_mass);
    auto alpha = QuadraticFunction(x, fCoefAlpha);
    return QuadraticFunction(x, fCoefCQ) * pow(DensityFraction(density), alpha);
}

double OsetCrossSection::PXSecCommon(const double pionE, const double pion_mass, const double cms_mom, const double fermimom, const double sqrts, const double density) const {
    //Compute delta prop sq
    auto pionKE = pionE - pion_mass;
    auto pion_momentum = sqrt(pionE*pionE - pion_mass*pion_mass);
    auto reduced_halfwidth = ReducedHalfWidth(pionE, pion_mass, cms_mom, fermimom, sqrts);
    // real and imaginary part of delta denominator (see eq. 2.23)
    const double re_delta = sqrts - Constant::mdelta;
    const double im_delta = reduced_halfwidth + SelfEnergyAbsNN(pionKE, pion_mass, density) + SelfEnergyAbsNNN(pionKE, pion_mass, density) + SelfEnergyQE(pionKE, pion_mass, density);

   
    // Compute delta proopagator squared
    auto delta_propagator2 = 1.0 / (re_delta * re_delta + im_delta * im_delta);
    
    auto pXsecCommon = fNormFactor * CouplingFactor(pion_mass) * delta_propagator2 * pow(cms_mom,2) / pion_momentum;

    return pXsecCommon;
}

double OsetCrossSection::ReducedHalfWidth(const double pionE, const double pion_mass, const double cms_mom, const double fermimom, const double sqrts) const {
    ...
    if(mu0 < -1.0) delta_reduction = 0.0;
    else if(mu0 > 1.0) delta_reduction = 1.0;
    else delta_reduction = (mu0 * mu0 * mu0 + mu0 + 2.) / 4.0;

    auto reduced_halfwidth = fConstFactor * coupling_factor * Constant::mN * cms_mom * cms_mom * cms_mom / sqrts * delta_reduction;
