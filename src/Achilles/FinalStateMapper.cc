...
double ThreeBodyMapper::GenerateWeight(const std::vector<FourVector> &mom, std::vector<double> &rans) {
    // The momentum are given in the following order:
    // 1. Momentum of the initial hadron
    // 2. Momentum of the initial lepton
...
double ThreeBodyMapper::MassivePropagatorMomenta(double mass, double width, double smin, double smax, double ran) {
    ...
    if(!(s>0) && !(s<0) && s!=0) {
        spdlog::debug("MassivePropMomenta produced a nan");
    }

    return s;
}

void ThreeBodyMapper::Isotropic2Momenta(FourVector p, double s1_, double s2_, FourVector &p1, FourVector &p2, double ran1, double ran2, double ctmin, double ctmax) {
...
int ThreeBodyMapper::TChannelMomenta(FourVector p1in, FourVector p2in, FourVector &p1out, FourVector &p2out, double s1out,double s2out, double t_mass, double ctexp, double ctmax, double ctmin, double aminct, int aminctflag, double ran1, double ran2) {
...
double ThreeBodyMapper::Tj1(double cn,double amcxm,double amcxp,double ran) {
    double ce= 1.-cn;
    double res = 0.;
    if (ce > 1.e-12) res = pow(ran*pow(amcxm,ce)+(1.-ran)*pow(amcxp,ce),1./ce);
    else {
...
double ThreeBodyMapper::MassivePropWeight(double mass, double width, int lim, double smin, double smax, double s, double &ran) {
...
double ThreeBodyMapper::TChannelWeight(const FourVector &p1in, const FourVector &p2in, const FourVector &p1out, const FourVector &p2out, double t_mass, double ctexp, double ctmax, double ctmin, double aminct, int, double &ran1, double &ran2) {
    ...
    aminct = a-ct;
    double wt = 2.*sabs/(-pow(aminct,ctexp)*Hj1(ctexp,a-ctmin,a-ctmax)*p1outmass*M_PI);

    return wt;

}

double ThreeBodyMapper::Isotropic2Weight(const FourVector& p1,const FourVector& p2,
