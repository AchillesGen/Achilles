...
void LegendreCoefficients(std::size_t lmax, std::array<std::array<double, 12>, 12> &A) {
    ...
    lmax = std::min(lmax, static_cast<std::size_t>(10));
    ...
    for(std::size_t n = 7; n < lmax + 1; n++) {
        std::size_t k0 = n % 2 + 2;
        for(std::size_t k = k0; k <= n; k += 2) {
...
void H_G_Coefficients(std::size_t n, std::size_t m, const std::array<std::array<double, 12>, 12> &A,
    ...
    for(std::size_t kn = k_n0; kn <= n; kn += 2) {
        for(std::size_t km = k_m0; km <= m; km += 2) {
            ...
            if(km > 0 && kn > 0) {
                ...
                g[km + kn] -=
                    -kmkn * A[n][kn] * A[m][km]; // redundant in principle, could do this in the end
            }
        }
    }
}
