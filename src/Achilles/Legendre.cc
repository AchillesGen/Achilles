...
void LegendreCoefficients(int lmax, std::array<std::array<double, 12>, 12> &A){
	...
	for (int n = 7 ; n < lmax + 1 ; n++)
	{
		int k0 = n%2 + 2;
		for (int k = k0 ; k <= n ; k+=2)
		{
			double dk = k*1.;
			A[n][k] = -1.*A[n][k-2]*(n*1. - dk + 2)*(n*1. + dk - 1)/dk/(dk-1);  
		}
...
void H_G_Coefficients(int n, int m, const std::array<std::array<double, 12>, 12> &A,  std::array<double, 26> &h, std::array<double, 26> &g)
{
	...
	for (int kn = k_n0 ; kn <= n ; kn+=2)
	{
		for (int km = k_m0 ; km <= m ; km+=2)
		{
			h[km+kn] += A[n][kn]*A[m][km];
			
			if (km > 0 && kn > 0)
			{
				g[km+kn - 2] += km*kn*A[n][kn]*A[m][km];
				g[km+kn] += -1.*km*kn*A[n][kn]*A[m][km]; //redundant in principle, could do this in the end
