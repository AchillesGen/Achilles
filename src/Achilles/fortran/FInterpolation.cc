...
achilles::Interp1D *CreateInterp1D(const double *x, const double *y, int n, int mode) {
    ...
    switch(mode) {
    case 0:
        interp = new achilles::Interp1D(xVec, yVec, achilles::InterpolationType::NearestNeighbor);
        break;
    case 1:
    case 2:
    case 3:
        interp = new achilles::Interp1D(xVec, yVec, achilles::InterpolationType::Polynomial);
        interp->SetPolyOrder(static_cast<size_t>(mode));
        break;
    case 4:
        interp = new achilles::Interp1D(xVec, yVec, achilles::InterpolationType::CubicSpline);
        interp->CubicSpline();
        break;
    default:
...
achilles::Interp2D *CreateInterp2D(double *x, double *y, double *z, int n1, int n2, int mode) {
    ...
    case 0:
        interp =
            new achilles::Interp2D(xVec, yVec, zVec, achilles::InterpolationType::NearestNeighbor);
        break;
    case 1:
    case 2:
    case 3:
        interp = new achilles::Interp2D(xVec, yVec, zVec, achilles::InterpolationType::Polynomial);
        interp->SetPolyOrder(static_cast<size_t>(mode), static_cast<size_t>(mode));
        break;
    case 4:
        interp = new achilles::Interp2D(xVec, yVec, zVec, achilles::InterpolationType::CubicSpline);
...
double Interpolate2D(achilles::Interp2D *interp, double x, double y) {
    ...
    } catch(std::domain_error &e) { return 0; }
}
}
