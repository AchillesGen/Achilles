program response_ia
  ...
  use quasi_el
  implicit none
    integer*4, parameter :: nev=2000000,neq=100,nvoid=50
    ...
    real*8, allocatable :: w(:),wt(:),sigma(:)
    real*8, allocatable :: sig(:,:),sig_err(:,:)
    real*8 :: ee,thetalept,coste,Q2,eef,sig_p,sig_n,norm,mqef,dummy
...
!    ti=MPI_Wtime()
    ...
    fname='results/C12_'//trim(en_char)//'.out'
    fname=trim(fname)
...
! construct omega grid and Q2 grid
    allocate(w(nw),wt(nw),sig(2,nw),sig_err(2,nw))
    wmin=0.0d0
...
!..start the real calculation using previously generated configurations         
          ...
          do iv=1,nev
             ...
             !call setrn(irn(j))
                ...
                if (iv.ge.neq.and.mod(iv,nvoid).eq.0) then
                   ...
                   coste=-1.0d0+2.0d0*ran()
                   thetalept=acos(coste)

                   eef = ee - w(iw)
                   Q2 = 2.0d0*ee*eef*(1.0d0 - coste)
                   qval=sqrt(Q2+w(iw)**2)
                   phi=2.0d0*pi*ran()
       ...
       if (myrank().eq.0) then
         ...
         write(6,*) w(iw),sum(sig(:,iw)),sum(sig_err(:,iw))
         write(14,*) w(iw),sum(sig(:,iw)),sig(1,iw),sig(2,iw)
         flush(14)
      endif
    enddo
       close(14)
