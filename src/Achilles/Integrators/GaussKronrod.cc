...
const std::vector<double> GaussKronrod::absc = {
...
double GaussKronrod::Integrate(const double &a, const double &b, double &err) {
    ...
    double absDiffIntegral =
        KronrodWgts[KronrodWgts.size() - 1] * (std::abs(fCenter - resultMeanKronrod));
    for(size_t j = 0; j < KronrodWgts.size(); ++j) {
        double abscissa = halfLength * absc[j];
        absDiffIntegral +=
            std::abs(Function(center - abscissa) - resultMeanKronrod) * KronrodWgts[j];
        absDiffIntegral +=
            std::abs(Function(center + abscissa) - resultMeanKronrod) * KronrodWgts[j];
    }
...
std::vector<double> GaussKronrod::IntegrateVec(const double &a, const double &b, double &err) {
    double halfLength = (b - a) / 2;
    double center = (a + b) / 2;
    ...
    std::for_each(resultGauss.begin(), resultGauss.end(),
                  [](double &x) { x *= GaussWgts[GaussWgts.size() - 1]; });

    std::for_each(resultKronrod.begin(), resultKronrod.end(),
                  [](double &x) { x *= KronrodWgts[KronrodWgts.size() - 1]; });
    ...
    for(size_t j = 0; j < KronrodWgts.size(); ++j) {
        ...
        for(size_t i = 0; i < f1.size(); ++i) {
            absDiffIntegral[i] += std::abs(f1[i] - resultMeanKronrod[i]) * KronrodWgts[j];
            absDiffIntegral[i] += std::abs(f2[i] - resultMeanKronrod[i]) * KronrodWgts[j];
        }
    }
