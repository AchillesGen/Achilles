...
std::vector<double> Adzint::IntegrateVec(const double &, const double &, const double &,
                                         const double &) {
    throw std::runtime_error("Not implemented!");
}

double Adzint::Integrate(const double &a, const double &b, const double &rerr, const double &aerr) {
    res = 0;
    ers = 0;
    ier = 0;
    double ddx = b - a;
    ...
    if(ier == 1) spdlog::warn("Maxint reached (target, error): {}, {}", target, ers);
