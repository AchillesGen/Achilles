...
achilles::FourVector achilles::SolveDelta(const achilles::FourVector &p1, const achilles::FourVector &p2,
...
std::pair<double, double> achilles::FindMomentumRange(const achilles::FourVector &q_free, double rangeExtend) {
    std::pair<double, double> range;
    const double detM = sqrt((q_free.M2()-4*pow(achilles::Constant::mN, 2))*(q_free.M2() + q_free.P2())/q_free.M2());
    range.first = q_free.M2() > 2*achilles::Constant::mN*q_free.E() ? (detM-q_free.P())/2 : (q_free.P()-detM)/2;
