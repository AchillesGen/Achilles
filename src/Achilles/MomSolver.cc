...
achilles::FourVector achilles::SolveDelta(const achilles::FourVector &p1,
    ...
    const double s = (p1 + p2).M2().native();
    ...
    auto _func = [&](double energy) {
        ...
        FourVector p3 = FourVector::FromNative(
            {energy, p3Mag * sinTheta * cos(phi), p3Mag * sinTheta * sin(phi), p3Mag * cosTheta});
        return s - 2 * (pcm * p3).native() + m3 * m3 - m4 * m4;
    };

    Brent brent(_func);
    double energy = brent.CalcRoot(m3, (p1 + p2).E().native());
    double p3Mag = sqrt(energy * energy - m3 * m3);
    return FourVector::FromNative(
               {energy, p3Mag * sinTheta * cos(phi), p3Mag * sinTheta * sin(phi), p3Mag * cosTheta})
        .Boost(beta);
}
...
achilles::FourVector achilles::SolveDeltaWithPotential(const achilles::FourVector &q,
    ...
    auto _func = [&](double cosTheta) {
        ...
        FourVector p3 = FourVector::FromNative(
            {energy, p3Mag * sinTheta * cos(phi), p3Mag * sinTheta * sin(phi), p3Mag * cosTheta});
        auto p4 = q - p3;
        auto potential4 = potential(p4.P().native(), radius4);
        p4.E() = units::Energy{sqrt(p4.P2().native() + pow(m4 + potential4.rscalar, 2)) +
                               potential4.rvector};
        return (q.E() - p3.E() - p4.E()).native();
    };
